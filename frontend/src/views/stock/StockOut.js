...
let headers = [
  ...
  // { label: "Stok Dikurangkan", key: "stock_added" },
...
const StockOut = () => {
  ...
  const fetchFirstData = async (url, authorization = "") => {
    ...
    (async () => {
      try {
        const data_stocks_in = fetchFirstData(
          `${process.env.REACT_APP_BE_URL_PORT}/web/stocks/list-detail-logs?sort=${sort}&search=${search}&status_stock_logs=0`,
          ""
        );
  ...
  const buttonSaveStock = async () => {
    if (
      dataItem.product_id == 0 ||
      !dataItem.stock 
    ) {
      return ToastDataSaveFailed("Produk, dan stock harus di isi");
    }

    if (Math.sign(dataItem.stock) !== -1) {
      return ToastDataSaveFailed(
        "Stok yang keluar harus mengandung input stok yang mengandung angka negative"
      );
    }
    ...
    await axios
      ...
      .catch((err) => {
        ToastDataSaveFailed(Array.isArray(err.response.data.message) ? err.response.data.message.join(' ') : err.response.data.message);
      });
  };
          ...
          <CCard>
            ...
            <CModal show={modal} onClose={closeModal}>
              <CModalHeader closeButton>
                <CModalTitle>Tambah Stok Keluar</CModalTitle>
              </CModalHeader>
              <CModalBody>
                <CForm>
                  <div>
                  ...
                  <div className="mt-2">
                    <CLabel htmlFor="stock">Stok Dikurangkan :</CLabel>
                    <CInput
                      onChange={(e) =>
                  ...
                  <div className="mt-2">
                    <CLabel htmlFor="suppliers_name">Suppliers :</CLabel>
                    <CSelect
                      onChange={(e) =>
            ...
            <CCardHeader>Tabel Stok Keluar</CCardHeader>
            <CCardBody>
              <div className="d-flex justify-content-between mb-3">
                ...
                <div className="d-flex">
                  ...
                  <CSVLink
                    filename="list-stock-out-postme"
              ...
              <CDataTable
                ...
                scopedSlots={{
                  ...
                  // stokDikurangkan: (item) => (
                  stokSetelahnya: (item) => (
                    <td className="stock-field-stock-after">
                      {/* <p>{item.stock_added}</p> */}
