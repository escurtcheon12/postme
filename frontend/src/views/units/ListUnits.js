...
const fields = ["No", "nama", "diBuat", "action"];
...
const ListUnits = () => {
  ...
  useEffect(() => {
    (async () => {
      await axios
        .get(
          `${process.env.REACT_APP_BE_URL_PORT}/web/unit/list?sort=${sort}&search=${search}`,
          {
            headers: {
  ...
  const editUnits = async (id) => {
    await axios
      .post(
        `${process.env.REACT_APP_BE_URL_PORT}/web/unit/update`,
        { id, name: dataItem.name },
        {
          headers: {
            Authorization: `Bearer ${
              localStorage.getItem("authorization_token") || ""
  ...
  const deleteUnits = async (id) => {
    await axios
      .post(
        `${process.env.REACT_APP_BE_URL_PORT}/web/unit/delete`,
        { id },
        {
          headers: {
            Authorization: `Bearer ${
        ...
        <CCol>
          ...
          <CCard>
            ...
            <CModal show={modalEdit} onClose={setModalEdit}>
              ...
              <CModalFooter>
                <CButton
                  className="button-edit-unit"
                  onClick={() => editUnits(dataItem.id)}
                >
            ...
            <CCardHeader>Tabel Unit</CCardHeader>
            <CCardBody>
              <div className="d-flex justify-content-between mb-3">
                ...
                <div className="d-flex">
                  <CSVLink
                    filename="list-unit-postme"
                    className="text-white"
                    data={data}
                    headers={headers}
                  >
                    <CButton className="button-csv-unit">
                      <FontAwesomeIcon className="mr-2" icon={faFileExport} />
                      CSV
                    </CButton>
                  </CSVLink>
                  <CButton
                    className="button-filter-unit ml-2"
